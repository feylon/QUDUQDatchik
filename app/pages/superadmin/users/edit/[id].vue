        ...
        <UCard>
            ...
            <div class="grid gap-8 md:grid-cols-2 grid-cols-1">
                ...
                <div class="flex max-w-[350px] gap-4 items-center justify-between mt-4">
                    ...
                    <UInput v-model="formData.password" type="text" :required="true" class="w-[250px]"
                        color="secondary" />
                </div>
                ...
                <div v-for="item in formData.users" class="flex max-w-[350px] gap-4 items-center justify-between mt-4">
                    <label><span class="font-semibold">Foydalanuvchi login</span></label>

                    <USelectMenu v-model="item.id" :items="users" value-key="id" label-key="label" class="w-[250px]" />
                </div>



                <div class="col-span-2 font-semibold flex justify-end">
                    <UButton @click="addUser" color="secondary">
...
const addUser = () => {
  const last = formData.users[formData.users.length - 1];

  // 1) Oxirgi tanlanmagan bo‘lsa — ogohlantirish
  if (!last || !last.id || last.id === '') {
    toast.add({
      title: "Avval foydalanuvchini tanlang",
      color: "error"
    });
    return;
  }

  // 2) Dublikat ID tekshirish
  const ids = formData.users.map(u => u.id).filter(id => id !== '');
  const unique = new Set(ids);

  if (unique.size !== ids.length) {
    toast.add({
      title: "Bu foydalanuvchi allaqachon tanlangan!",
      color: "warning"
    });

    // Dublikat bo‘lsa yangi user qo‘shilmaydi
    return;
  }

  // 3) Hammasi yaxshi bo‘lsa yangi user qo‘shish
  formData.users.push({ id: '' });
};
